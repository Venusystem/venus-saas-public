...
@app.get("/")
def root(): return {"message": "Venus ERP Sync v4 Live"}
    ...
    with engine.connect() as conn:
        for r in p.data:
            conn.execute(text("""
                ...
                ON CONFLICT (tenant_id, local_id) DO UPDATE SET name=EXCLUDED.name, selling_price=EXCLUDED.selling_price, opening_stock=EXCLUDED.opening_stock
            """), {"tid": p.tenant_id, "lid": r['local_id'], "name": r['name'], "code": r['code'], "cat": r['category'], "cp": r['cost'], "sp": r['price'], "ws": r['wholesale'], "op": r['stock']})
        conn.commit()
...
def sync_sales(p: Payload):
    with engine.connect() as conn:
        for r in p.data:
            ...
            """), {"tid": p.tenant_id, "lid": r['local_id'], "date": r['date'], "store": r['store'], "cust": r['customer'], "cash": r['cashier'], "pay": r['paymode'], "tot": r['total']})
            
            for i in r.get('items', []):
                conn.execute(text("INSERT INTO sale_items (id, tenant_id, sale_local_id, item_name, quantity, price, total, cog) VALUES (gen_random_uuid(), :tid, :sid, :nm, :qt, :pr, :tot, :cog)"),
                             {"tid": p.tenant_id, "sid": r['local_id'], "nm": i['name'], "qt": i['qty'], "pr": i['price'], "tot": i['total'], "cog": i['cog']})
        conn.commit()
    return {"status": "success"}
